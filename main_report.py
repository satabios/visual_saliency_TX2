...
if __name__ == '__main__':
    ...
    while (True):
        ...
        cv2.imshow('frame',cv2.applyColorMap((((-1 * salmap) / 32) * 255).astype(np.uint8),cv2.COLORMAP_JET))
        # cv2.imshow('frame',cv2.medianBlur(cv2.resize((cv2.applyColorMap((((-1 * salmap) / 32) * 255).astype(np.uint8), cv2.COLORMAP_JET)), (width, height), interpolation=cv2.INTER_LINEAR),10))
