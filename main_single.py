...
if __name__ == '__main__':
    ...
    video = np.asarray(np.zeros((6, height, width, 3)))
    start = time.time()
    for i in range(6):
        ret, frame = cap.read()
        print(np.asarray(frame).shape)
        video[i, :, :, :] = frame
    imgs = np.asarray(np.zeros((video[0].shape[0], video[0].shape[1], video[0].shape[2], len(params['channels']))))
    # plt.ion()
    ...
    while (True):
        ...
        #print("sahep:",np.asarray(normalizeImage(temp_out_strong).shape,"pop",imgs[:, :, :, 0].shape,type(imgs[:, :, :, 0]))#.shape) 
        print(normalizeImage(temp_out_strong).shape)
        imgs[:,:,:,0] = np.asarray(normalizeImage(temp_out_strong))
